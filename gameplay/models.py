...
class Game(models.Model):
    ...
    def board(self):
        ...
        so you can ask for the state of a square of position [y][x]"""
    ...
    def new_move(self):
        if self.status not in 'FS':
            raise ValueError("Can not make a move on a finished game")
...
class Move(models.Model):
    ...
    def __str__(self):
        return "{0}, x:{1}, y:{2}".format(self.comment, self.x, self.y)
